## traffic_engine/build_review_pages.py
import json
import datetime
import html
...
def render_review_page(tool, tools_list, template):
    ...
    affiliate_link = html.escape(tool.get("affiliate_link", "#"))
    slug = tool["slug"]
    ...
    review_summary = html.escape(tool.get("review_summary", tool.get("short_summary", f"{name} is an AI productivity tool.")))
    features = render_list_items(tool.get("features", ["AI-powered productivity enhancement"]))
    pros = render_list_items(tool.get("pros", [f"May save time for active users", f"Accessible price point", "Can fit well into workflows"]))
    cons = render_list_items(tool.get("cons", ["Requires setup and habit changes", f"Recurring monthly cost", "Real value depends on usage"]))
    ...
    pricing_note = tool.get("pricing_note", "")
